...
/** Removes unneccesary components for Evil X Boss that are generated by default:
 * - Extra rows and their board slots
 * - Boss's item slots
 */
export function setupEvilXisumaBossBoard(components: ComponentTable): void {
	const destroyRow = (row: RowEntity) => {
		// First delete all slots in the row
		components
			.filter(BoardSlotComponent, query.slot.rowIs(row))
			.forEach((slot) => components.delete(slot.entity))
	...
	components
		...
		.forEach((row) => destroyRow(row.entity))
	...
	components.filter(RowComponent, query.row.currentPlayer).forEach((row) => {
		row.itemsSlotEntities?.forEach((slotEntity) =>
			components.delete(slotEntity),
		)
