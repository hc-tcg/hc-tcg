...
class BeetlejhostRare extends Card {
	...
	override onAttach(game: GameModel, component: CardComponent, observer: ObserverComponent): void {
		...
		observer.subscribe(player.hooks.afterAttack, (attack) => {
			...
			const chromakeyed = game.components.filter(StatusEffectComponent, query.effect.targetEntity(component.entity), query.effect.id('chroma-keyed'))[0]
			if (!chromakeyed) {
				game.components
					.new(StatusEffectComponent, ChromaKeyedEffect)
