...
test('Game starts for players and spectators and places players back on main menu.', async ({
	...
	await spectator.getByLabel('Player Name').fill('Test Player')
	await spectator.getByLabel('Player Name').press('Enter')
	...
	for (const player of [playerOne, playerTwo]) {
		await player.getByLabel('Player Name').fill('Test Player')
		await player.getByLabel('Player Name').press('Enter')
