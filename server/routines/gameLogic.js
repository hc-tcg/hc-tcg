...
function* sendGameState(game, derivedState) {
	...
	gamePlayerIds.forEach((playerId) => {
		game.players[playerId].socket.emit('GAME_STATE', {
			type: 'GAME_STATE',
			payload: {
 ...
 * @param {Game} game
 */
function* gameSaga(root, game) {
	// @TODO eventually this method should not receive root at all, and it should be
	...
	turn_cycle: while (true) {
		game.state.turn++
		const result = yield call(turnSaga, root, game)
