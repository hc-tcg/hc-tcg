...
import {PickRequirmentT, PickResultT, PickedSlotT, SlotTypeT} from 'common/types/pick-process'
...
const checkRow = (
	...
	if (req.adjacent === 'active') {
		...
		if (!(rowInfo.index === activeRow - 1 || rowInfo.index === activeRow + 1)) return 0
 ...
 * @return {Array<*>}
 */
const getRowsInfo = (playerState: PlayerState | LocalPlayerState, current: boolean): Array<any> => {
 ...
 * @returns {number}
 */
const checkHand = (gameState: LocalGameState, req: PickRequirmentT): number => {
	let cards = gameState.hand
	if (req.type) {
		cards = cards.filter((card) => req.type && req.type.includes(CARDS[card.cardId].type))
	}
...
export const anyAvailableReqOptions = (
	...
	opponentState: LocalPlayerState | null,
	reqs: Array<PickRequirmentT>
): Array<number> => {
	const result: any = []
	if (!gameState || !playerState || !opponentState) return result

	const rowsInfo: Array<any> = []
...
export const validRow = (
	cardPlayerState: PlayerState | LocalPlayerState,
 ...
 * @returns {boolean}
 */
export const validActive = (
	active: PickRequirmentT['active'],
	cardPlayerState: PlayerState | LocalPlayerState,
	rowIndex: number | null
): boolean => {
	...
	const isActive = hasActiveHermit && rowIndex === cardPlayerState?.board.activeRow
 ...
 * @returns {boolean}
 */
export const validType = (type: PickRequirmentT['type'], cardType: CardTypeT | null): boolean => {
	if (!type) return true
 ...
 * @returns {boolean}
 */
export const validSlot = (type: PickRequirmentT['slot'], slotType: SlotTypeT): boolean => {
	if (!type) return true
 ...
 * @returns {boolean}
 */
const validEmptyRow = (emptyRow: PickRequirmentT['emptyRow'], isEmptyRow: boolean): boolean => {
 ...
 * @returns {boolean}
 */
const validRemovable = (removable: PickRequirmentT['removable'], card: CardT | null): boolean => {
 ...
 */
const validAdjacent = (
	adjacent: PickRequirmentT['adjacent'],
	gameState: GameState | LocalGameState,
	...
	if (
		activeRow !== null &&
		(pickedSlot.row.index === activeRow + 1 || pickedSlot.row.index === activeRow - 1)
	)
		return true
...
export function validPicks(
	gameState: GameState | LocalGameState,
	pickResults: PickResultT[]
): boolean {
	const reqAdjacents: Array<Array<PickedSlotT>> = []
	const boardSlots: Array<PickedSlotT> = []
	for (const result of pickResults) {
		const pickedSlotsForReq = result.pickedSlots
		const req = result.req
		const reqAdjacentsBundle: Array<PickedSlotT> = []
			...
			for (let pickedSlotAdj of pickedSlotAdjBundle) {
				...
				for (let pickedSlot of boardSlots) {
					...
					if (
						slotAdj.row &&
						slot.row &&
						(slot.row.index === slotAdj.row.index + 1 || slot.row.index === slotAdj.row.index - 1)
					) {
						validPairs++
						break
					}
