		...
		for (let rowIndex in playerRows) {
			...
			if (row.hermitCard && row.health <= 0) {
				...
				playerRows[rowIndex] = {
					hermitCard: null,
					health: null,
					itemCards: new Array(row.itemCards.length), // A row should have the same number of item cards upon Hermit death
					effectCard: null,
				}
...
function* sendGameState(game: BossModel) {
...
function* turnActionsSaga(game: BossModel) {
	const {opponentPlayer, opponentPlayerId, currentPlayer, currentPlayerId, isBossTurn} = game
		...
		while (true) {
			...
			if (isBossTurn) {
				if (game.state.turn.turnNumber == 2) {
					const bossCard = currentPlayer.hand[0]
					if (bossCard) {
						...
						yield* call(turnActionSaga, game, playBossCard)
						continue
						//yield* fork(put, turnActionChannel, playBossCard)
					}
				}
				const attackType = availableActions.find((action) => {
					return action == 'PRIMARY_ATTACK' || action == 'SECONDARY_ATTACK'
				})
				if (attackType) {
					const attackAction = {
						type: attackType,
					...
					yield* call(turnActionSaga, game, attackAction)
					continue
				}
			}
...
function* turnSaga(game: BossModel) {
	...
	if (drawCards.length === 0 && !game.isBossTurn) {
...
function* bossSaga(game: BossModel) {
