    ...
    const result = yield* race({
      ...
      playerRemoved: take(
        (action: any) =>
          ...
          playerIds.includes((action as LocalMessageTable[typeof localMessages.PLAYER_REMOVED]).player.id),
      ),
      forfeit: take(
        (action: any) =>
          action.type === clientMessages.FORFEIT &&
          playerIds.includes((action as RecievedClientMessage<typeof clientMessages.FORFEIT>).playerId),
      ),
    })
